    ...
    if (!isAdmin) {
      if (isRequestOwner) {
        ...
        if (userId === clientId) {
          ...
          const recipient = await db.collection('users_public').doc(userId).get();
...
exports.incrementTrustReviewStats = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Authentication required.'
  ...
  if (
    quality == null || quality < 1 || quality > 5 ||
    communication == null || communication < 1 || communication > 5 ||
    onTime == null || onTime < 1 || onTime > 5 ||
    deliverySpeed == null || deliverySpeed < 1 || deliverySpeed > 5
  ) {
    throw new functions.https.HttpsError(
      'failed-precondition',
  ...
  const eventRef = db.collection('trust_events').doc(
    `review_${bookingId}_${context.auth.uid}`
  );
...
exports.incrementTrustCompletedBookings = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Authentication required.'
  ...
  const eventRef = db.collection('trust_events').doc(`completed_${bookingId}`);
  const statsRef = db.collection('trust_stats').doc(photographerId);
  await db.runTransaction(async (tx) => {
    const existing = await tx.get(eventRef);
    if (existing.exists) return;
...
exports.incrementTrustCanceledByPhotographer = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Authentication required.'
  ...
  if (booking.data.status !== 'canceled' || booking.data.canceledBy !== context.auth.uid) {
    throw new functions.https.HttpsError(
      'failed-precondition',
...
exports.incrementTrustDisputesCount = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Authentication required.'
...
exports.updateReelLike = functions.https.onCall(async (data, context) => {
  ...
  const likeRef = db.collection('reel_likes').doc(reelId).collection('users').doc(userId);
